...
def process_file(file: str, output_folder: str):
    ...
    try: 
        ...
        length = len(sequence)
        input_matrix = prepare_data.make_matrix_from_sequence_16(sequence)
...
if __name__ == "__main__": 
    ...
    try: 
        ...
        process_and_save(file_list, "data/complete_set16")
    ...
    file_list = [os.path.join('data', 'complete_set16', file) for file in os.listdir('data/complete_set16')]

    train, valid, test = prepare_data.split_data(file_list, validation_ratio=0.1, test_ratio=0.1)

    pickle.dump(train, open('data/train16.pkl', 'wb'))
    pickle.dump(valid, open('data/valid16.pkl', 'wb'))

    print("Move test files to different folder")
    os.makedirs("data/test_files16", exist_ok=True)
    for file in test: 
        shutil.move(file, "data/test_files16")

    test = [os.path.join('data', 'test_files16', os.path.basename(file)) for file in test]    
    pickle.dump(test, open('data/test16.pkl', 'wb'))
    ...
    plots.plot_families({"train":train, "valid":valid, "test":test}, output_file='figures/family_distribution16.png')
    plots.plot_len_histogram({"train":train, "valid":valid, "test":test}, output_file='figures/length_distribution16.png')

    print("DONE", file=sys.stdout)
    print(f"Size of folder for training: {get_folder_size('data/complete_set16'):.2f} GB", file=sys.stdout)
    print(f"Size of folder for testing: {get_folder_size('data/test_files16'):.2f} GB", file=sys.stdout)
