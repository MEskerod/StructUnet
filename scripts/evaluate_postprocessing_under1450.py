...
if __name__ == "__main__":
    ...
    file_list = pickle.load(open('data/valid_under_1450.pkl', 'rb'))
    ...
    # Save the results
    df.to_csv('results/evalutation_postprocess_under1400.csv', index=False)

    # Plot the results
    f1 = df[df.filter(regex='f1').columns]
    f1 = f1.apply(pd.to_numeric, errors='coerce')
    violin_plot(f1, 'Post-processing methods', outputfile='figures/evaluation_postprocess_uder1400.png') 
    ...
    results.to_csv('results/average_scores_postprocess_under1400.csv')
