...
def from_numpy_array(A: np.array):
    ...
    if A.ndim != 2:
        raise ValueError(f"Input array must be 2D, not {A.ndim}")
    n, m = A.shape
    if n != m:
        raise ValueError(f"Adjacency matrix not square: nx,ny={A.shape}")

    # Make sure we get even the isolated nodes of the graph.
    G.add_nodes_from(range(n))

    # Get a list of all the entries in the array with nonzero entries. These coordinates become edges in the graph. (convert to int from np.int64)
    # Only add the edges from the upper triangle of the matrix, since it's a undirected graph
    edges = ((int(e[0]), int(e[1])) for e in zip(*A.nonzero()))
    
    triples = ((u, v, {"weight": A[u, v]}) for u, v in edges if u <= v)
...
class Graph:
    ...
    def __str__(self):
        ...
        -------
        info : string
            Graph information including graph type, and the
            number of nodes and edges.
        """
        return "".join([type(self).__name__,
                        f" with {len(self)} nodes and {int(sum([len(self._adj[n])+(n in self._adj[n]) for n in self._adj]) // 2)} edges"])
    ...
    @cached_property
    def nodes(self):
        """A list of the Graph as G.nodes.
    ...
    def edges(self):
        ...
        Returns
        -------
        edges : A view of edge attributes, it iterates over (u, v, d) tuples of edges, but can also be used for
            attribute lookup as `edges[u, v]['weight']`.
        """
        seen = {}
        edges = []
...
def max_weight_matching(G):
    ...
    class Blossom:
        ...
        # b.childs is an ordered list of b's sub-blossoms, starting with the base and going round the blossom.

        # b.edges is the list of b's connecting edges, such that b.edges[i] = (v, w) where v is a vertex in b.childs[i]
        # and w is a vertex in b.childs[wrap(i+1)].

        # If b is a top-level S-blossom, b.mybestedges is a list of least-slack edges to neighbouring
